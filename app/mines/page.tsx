...
export default function MinesPage() {
  ...
  // Read saved progress once on mount (lazy ref, avoids multiple sessionStorage reads)
  ...
  // Show modal immediately if restoring a won game, otherwise delay after bombs
  const [showModal,    setShowModal]    = useState(() => _s.current?.phase === "won");
  // Skip bomb animation if restoring an already-won game
  ...
  // Show modal after bombs finish animating (~1.6s after win)
  useEffect(() => {
  ...
  // Reveal remaining cells as bombs after winning
  useEffect(() => {
  ...
  return (
    <>
      ...
      <main className="relative flex min-h-[100dvh] flex-col items-center bg-black px-4 pb-10 pt-[4.75rem]">
        {/* bg orbs */}
        ...
        <div className="flex w-full max-w-[26rem] flex-col items-center gap-5">
          ...
          {/* Win modal rendered via portal-like fixed layer */}
          {showModal && (
            <WinModal
